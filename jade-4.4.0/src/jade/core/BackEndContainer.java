...
public class BackEndContainer extends AgentContainerImpl implements BackEnd {
		...
		try {
			...
			if (connected) {
				...
				if ("true".equals(myProfile.getParameter(RESYNCH, "false"))) {
					...
					// Notify the main container about bootstrap agents on the
					// FE.
	...
	protected void startNode()
		...
		//**//
		System.out.println("Starting Container:" + myID.getName());
		//**//
		
		List l = myProfile.getSpecifiers(Profile.SERVICES);
		myProfile.setSpecifiers(Profile.SERVICES, l); // Avoid parsing services
														// twice
		...
		// Actually join the platform (this call can modify the name of this
		// container)
	...
	public String bornAgent(String name) throws JADESecurityException, IMTPException {
		name = JADEManagementOntology.adjustAgentName(name,
				new String[] { getID().getName(), myProfile.getParameter(Profile.AGENT_TAG, "") });
	...
	public void messageOut(ACLMessage msg, String sender) throws NotFoundException, IMTPException {
		...
		synchronized (frontEndSynchLock) {
			...
			if (image == null) {
				if (synchronizing) {
					// The image is not yet there since the front-end is
					// synchronizing.
	...
	public Object serviceInvokation(String actor, String serviceName, String methodName, Object[] methodParams)
		...
		//**//
		if (image == null){
			System.out.println("Could not find image for actor: " + actor);
		}
		//**//
				...
				if (!m.isAccessible()) {
					try {
						...
						throw new NoSuchMethodException(
								"Method " + methodName + "() of class " + getClass().getName() + " not accessible.");
					}
				}
	...
	private boolean isCompatible(Method method, Object[] params) {
		// FIXME: At present we just check the number of parameters. Implement
		// types compatibility
		...
		if (found) {
			...
			if (image != null) {
				if (agentImages.containsKey(msg.getSender()) && isExplicitReceiver(msg, receiverID)) {
					// The message was sent by an agent living in the FrontEnd.
					// The
					// receiverID (living in the FrontEnd too) has already
					// received
					// the message.
					// The second part of the condition ensures that, if the
					// message was not directly sent to the receiver (e.g. it
					// was sent to a topic
					// or an alias), message delivery occurs normally.
					// FIXME: This does not take into account that an agent not
					// living
					// in the FrontEnd may send a message on behalf of an agent
					// living
					// in the FrontEnd.
					return true;
				}
	...
	public boolean postMessagesBlockToLocalAgent(ACLMessage[] mm, AID receiverID) {
		// FIXME: To optimize performances we should support messages-block
		// transfer between BE and FE
		boolean ret = false;
	...
	public Agent acquireLocalAgent(AID id) {
		...
		if (ag == null) {
			// The agent is not in the LADT. Likely it is in the FE --> Acquire
			// its image
	...
	public void releaseLocalAgent(AID id) {
		// If the aquired agent was an image there is nothing to release (see
		// FIXME above)
	...
	public void shutDown() {
		...
		try {
			...
			// The FrontEnd is disconnected. Force the shutdown of the
			// connection
	...
	private void resynch() {
		...
		Thread synchronizer = new Thread() {
			public void run() {
				while (!terminating) {
					try {
						// Wait a bit also before the first attempt to avoid
						// sending the synch
						...
						// Since the synchronization process will be repeated,
						// be
						// sure we start from a clean situation
						// killAgentImages();

						// The input connection is down again (or there was an
						// IMTP
	...
	private void postponeAfterFrontEndSynch(ACLMessage msg, String sender) {
		// No need for synchronization since this is called within a
		// synchronized block
